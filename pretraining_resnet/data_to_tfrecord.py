...
tf.app.flags.DEFINE_string('train_data_dir',
                           '/home/zyq/dataset/LRW/video_frame/train',
                           'Train dataset')

tf.app.flags.DEFINE_string('val_data_dir',
                           '/home/zyq/dataset/LRW/video_frame/val',
                           'Val dataset')

tf.app.flags.DEFINE_string('label_dir',
                           '/home/zyq/dataset/LRW/lipread_mp4',
                           'Traing label directory')

tf.app.flags.DEFINE_string('output_dir',
                           '/home/zyq/dataset/LRW/tfrecord',
                           'Output directory')

tf.app.flags.DEFINE_string('train_shards', 16,
                           'Number of shards in training TFRecord files.')
tf.app.flags.DEFINE_string('val_shards', 1,
                           'Number of shards in training TFRecord files.')
...
tf.flags.DEFINE_string('word_counts_output_file',
                       '/home/zyq/dataset/LRW/tfrecord/word_counts.txt',
...
class Vocabulary(object):
    ...
    def _extract_charater_vocab(self, label_dir):
        '''get label_to_text'''
        dic = {}
        label_list = glob.glob(os.path.join(label_dir, '*'))
...
def get_label(vocab, path):

    text = os.path.basename(path).split('_')[0]
    label = vocab.word_to_id.get(text)
    return label


def _to_sequence_example(video_path, vocab):
    ...
    label = get_label(vocab, video_path)
...
def process_batch_files(thread_index, ranges, name, video_list,
    ...
    for s in range(num_shards_per_batch):
        ...
        for i in video_in_shard:
            ...
            sequence_example = _to_sequence_example(video_path, vocab)
...
def process_dataset(name, video_dir, vocab, num_shards):
    ...
    data_list = glob.glob(os.path.join(video_dir, '*'))
    ...
    spacing = np.linspace(0, len(data_list), num_threads + 1).astype(int)
    ranges = []
    threads = []
    ...
    for thread_index in range(len(ranges)):
        args = (thread_index, ranges, name, data_list, vocab, num_shards)
...
def main(unused_argv):
    ...
    vocab = Vocabulary(FLAGS.label_dir)

    process_dataset('train', FLAGS.train_data_dir, vocab, FLAGS.train_shards)
    process_dataset('val', FLAGS.val_data_dir, vocab, FLAGS.val_shards)
