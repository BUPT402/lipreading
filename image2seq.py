...
class Image2Seq:
    def __init__(self, depth, img_height, img_width, img_ch, word2idx, epoch_num, beam_width, keep_prob,
                 embedding_dim=512,
                 hidden_size=512,
                 batch_size=30,
                 n_layers=1,
    ...
    def add_encode_layer(self):
        ...
        with tf.name_scope('GRU'):
            ...
            self.encoder_state = tf.concat([enc_fw_state[1], enc_bw_state[1]], 1)

    def lstm_cell(self, resuse=False):
        return tf.nn.rnn_cell.LSTMCell(self.hidden_size, initializer=tf.orthogonal_initializer(), reuse=resuse)
    ...
    def add_decoder_for_training(self):
        ...
        # decoder表示一个decoder实例 ，maxinum_interations表示为最大解码步长，默认为None解码至结束，return9(final_outputs,final_state
    ...
    def add_backward_path(self):
        ...
        self.loss = tf.contrib.seq2seq.sequence_loss(
            logits=self.train_flag, targets=self.processed_decoder_output(), weigths=masks)
