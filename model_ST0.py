...
class Lipreading(object):
    ...
    def __init__(self, model_config, iterator, train_config, word2idx, sess=tf.Session()):
        ...
        self.summary_writer = tf.summary.FileWriter('logs_train/log' +
        ...
        with tf.variable_scope('conv3d') as scope:
            ...
            with tf.variable_scope('conv2'):
                conv2 = tf.layers.conv3d(maxp1, 64, [3, 5, 5], [1, 1, 1], padding='same',
                                         use_bias=True, kernel_initializer=self.initializer, name='conv2')
            ...
            with tf.variable_scope('conv3'):
                conv3 = tf.layers.conv3d(maxp2, 96, [3, 3, 3], [1, 1, 1], padding='same',
                                         use_bias=False, kernel_initializer=self.initializer, name='conv3')
    ...
    def buils_encode(self):
        with tf.variable_scope('encoder') as scope:
            with tf.name_scope('stack_bigru'):
                ...
                self.encoder_state = (state_1, state_1)
    ...
    def build_decode(self):
        ...
        with tf.variable_scope('decoder', reuse=True) as scope:
            ...
            self.predicting_decoder = tf.contrib.seq2seq.BeamSearchDecoder(
                ...
                output_layer=core_layers.Dense(len(self.word2idx),
                                               kernel_initializer=tf.contrib.layers.xavier_initializer(), _reuse=True),
    ...
    def train(self):
        _, loss = self.sess.run([self.train_op, self.loss], feed_dict={self.is_training: True, self.dropout_prob: 0.5})
        return loss
